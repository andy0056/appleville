...
function citedPaths(query: string) {
  return generateAssistantResponse(query).citations.map((c) => c.href.split("#")[0]);
}

test("personal safety outranks an incidental property word", () => {
  ...
  const paths = citedPaths("Is it okay for a single woman to rent alone there?");
...
test("a ranking question cites the guide that answers it, not only towns", () => {
  const paths = citedPaths("best Himachal towns for remote workers");
  assert.equal(
    paths[0],
...
test("a trial-move question cites the trial-move guide", () => {
  ...
  const paths = citedPaths("how to test a move before committing");
  assert.ok(
